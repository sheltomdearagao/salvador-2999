...
const CharacterSelection: React.FC = () => {
  ...
  return <div className="py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {characters.map(character => <motion.div key={character.id} className={`character-card ${gameState.selectedCharacter?.id === character.id ? 'character-card-active' : 'character-card-inactive'}`} onClick={() => selectCharacter(character)} initial="inactive" animate={gameState.selectedCharacter?.id === character.id ? "active" : "inactive"} whileHover="hover" variants={characterVariants} transition={{
        type: "spring",
        stiffness: 300,
        damping: 20
      }}>
            <div className="relative h-72 overflow-hidden bg-gray-100">
              <img src={character.imagePath} alt={character.name} className="w-full h-full object-contain object-center transition-transform duration-300" />
              {gameState.selectedCharacter?.id === character.id && <div className="absolute inset-0 bg-cyber-purple/20 flex items-center justify-center py-0">
                  <div className="bg-cyber-purple text-white rounded-full p-2">
                    <User className="h-8 w-8" />
                  </div>
                </div>}
            </div>
            <div className="p-4 bg-white py-[5px]">
              ...
              <div className="text-sm bg-cyber-blue/10 p-3 rounded-md border border-cyber-blue/30 py-0">
